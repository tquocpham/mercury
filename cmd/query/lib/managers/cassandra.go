...
type CassandraClient interface {
	RefreshMessages(conversationID string, messageID string) (*GetMessagesResponse, error)
	GetMessages(conversationID string, pageSize int, nextToken []byte) (*GetMessagesResponse, error)
	Close()
}

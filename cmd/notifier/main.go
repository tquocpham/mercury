...
func main() {
	...
	handler := handlers.NewWebSocketHandlers(redisClient)
	e := echo.New()
	v1 := e.Group("api/v1", middleware.UseLogger(logger, environment))
	v1.GET("/ws", handler.NotifyClient) // no statsd. it would throw off our metrics for long lived websocket connections
	v1.POST("/send", handler.SendNotification, middleware.UseStatsd(statsdClient))
